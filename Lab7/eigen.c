...
vec InverseIter( matr *m, vec *y0, double eps, int *iter )
{
  ...
  *iter = 0;
  ...
  FreeVec(&y_i_n);
  y_i_n = VecMulNum(&y_i, 1 / NormEucl(&y_i));
...
{
  vec eigs = Zeros(m->size), eigs_1 = Ones(m->size);
  matr l = Unit(m->size), u = Unit(m->size), ak = Unit(m->size), ak_1 = Unit(m->size), diffm = Unit(m->size);

  *iter = 0;

  CopyMatr(&ak, m);

  while (InfNorm(&diffm) > eps)
  {
    matr e = Unit(m->size);
    matr et = MatrMulNum(&e, eigs.v[m->size - 1]);
    ...
    if ((*iter) % 1 == 0)
    {
      LUDecompose(&ak, &l, &u);
      FreeMatr(&ak);
      ak = MatrMulMatr(&u, &l, 0);

      FreeMatr(&diffm);
      diffm = MatrMinusMatr(&ak, &u);
    }
    /* with shift */
    else
    {
      FreeMatr(&ak_1);
      /* Ak - a_nn * E */
      ak_1 = MatrMinusMatr(&ak, &et);
      /* Ak - a_nn * E = Lk * Uk */
      LUDecompose(&ak_1, &l, &u);
      
      FreeMatr(&ak_1);
      FreeMatr(&ak);
      /* Ak+1 = Uk * Lk + a_nn * E */
      ak_1 = MatrMulMatr(&u, &l, 0);
      ak = MatrPlusMatr(&ak_1, &et);

      FreeMatr(&diffm);
      diffm = MatrMinusMatr(&ak_1, &u);
    }
